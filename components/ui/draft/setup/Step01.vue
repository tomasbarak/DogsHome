<script setup lang="ts">
    ...
    const securityChecks = () => {
        ...
        if (profile.name.length < 2) {
            return {secure: false, message: 'El nombre es demasiado cortos.'};
        }

        if (profile.name.length > 50) {
            return {secure: false, message: 'El nombre es demasiado largos.'};
        }
    ...
    const handleStepSubmission = async () => {
        ...
        try {
            const response = await fetch(`${apiUrl}/user/draft/creation/step/1`, {
                method: 'POST',
                headers: {
...
<template>
    <div class="absolute w-full top-0 left-0 h-full z-0 flex flex-col justify-start items-center">
        ...
        <h1 class="font-['lato'] font-extrabold text-[24pt] text-[#333] max-w-full w-[75vw] md:w-[50vw] m-0 text-center md:text-[32pt]">¿Cómo se llama el perro?</h1>
        ...
        <AuthInput v-model="profile.name" placeholder="Mora" type="text"/>
